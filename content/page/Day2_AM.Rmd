...
# YOUR TURN! A sickness grows... 

> Practicing joining and summarizing
...
Follow the steps below to collect the current disease records and summarize the cases by planet. To do so, you will need to join the disease records to a separate table containing the planet information. 
...
- Name the project "starsick"

### Get the disease data {-}
  
<a href="data/disease_records.Xlsx"><button class="button-dl" download="disease_records.xlsx">
...
Use `read_xlsx()` from the `readxl` library to load the disease data into R. Add the code below to your R script and replace the blank with the file name to load the data.

```{r, eval=F}
library(readxl)

records <- read_xlsx("______")
```
...
```{r, eval=F}
summarize(people, n_homeworlds = n_distinct( _______ ))
```
...
> Is one of the homeworlds `NA`? What does the 'NA' mean?


### Join the homeworld column to the disease records {-}

Complete the script below to join the two data sets. 

Fill in the first blank with the name of the data you want to join to `records`. In the second blank, put the column name found in both data sets that you want to join **by**. This is the column that both tables have in common.

> You can run `names(records)` to view the column names in a dataframe.


```{r, eval=F}

joined <- left_join(records, _______, by = "______")
...
Use `group_by()` to group the records by `homeworld` planet.
...
Now you can use the `n()` function to find the sum total of records by `homeworld`.
...
Use `arrange()` to sort the results by the `cases` column. 
...
1. Use `write_csv()` to save the list as **"restricted_planets.csv"**.  
1. Save your R script.  
1. Clap 3 times for your great work.  
1. When you're ready, return to your `jakku` project.  
