...
An easy way to experiment with colors is to add layers like `+ scale_fill_brewer()` or `+ scale_color_brewer()` to your plot. These link to **colorBrewer** palettes to give you accessible color themes. See https://ggplot2.tidyverse.org/reference/scale_brewer.html.
</div>
...
You may not like the appearance of these plots. `ggplot2` uses `theme` functions to change the appearance of a plot. View the list of themes [here](#themes).
...
3. _Don't worry if it looks_ __really wrong__. _Consider it art and try again._  
