...
# Check that the .db file pops up in your working directory
conn <- dbConnect(RSQLite::SQLite(), "people.db")
```


```{r}
# Add the dataset into a table called "people_tbl"
...
- `port` : A 4 digit number identifies the port to use for communicating with the db  
- `db` : The db name  
...
Woah! That can be a lot to track down. Luckily, there are helper packages to simplify connecting to various databases. Contact your Agency's R user group for more info.


# <i class="fas fa-key"></i> My password - Is it secret, is it safe? 
<hr>
...
> RDS databases at MDH. TEMPO at MPCA.
...
#### You were an excellent Data Droid! {-}

**Rey and BB8 couldn't have done it without you.**  
