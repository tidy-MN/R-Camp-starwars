...
Let's use it to look at the rows in the data that have an item listed as `"Total"`.

```{r filter12, eval=T}
filter(scrap, item == "Total")
```
...
Let's look at the data without the _Total_ category. Look in the comparison table above to find the `NOT` operator. We're going to filter the data to keep only the items that are `NOT equal` to "Total".

```{r filter-all}
scrap <- filter(scrap, item != "Total")
```
